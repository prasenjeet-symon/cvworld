...
class NetworkMediaApi {
  ...
  final Uri _uriUploadProfilePicture = Uri.parse('${ApplicationConfiguration.apiUrl}/server/api/user/profile_picture');
  ...
  Stream<ApiResponse> uploadProfilePicture(Uint8List? imageBytes, String fileName) {
    if (imageBytes == null) {
      return const Stream<ApiResponse>.empty();
    }
    ...
    http.MultipartRequest request = http.MultipartRequest('POST', _uriUploadProfilePicture);
    ...
    return request.send().asStream().switchMap((streamedResponse) {
      ...
      if (event.statusCode == 200) {
        SuccessManager.getInstance().dispatch('Profile picture uploaded successfully.');
      } else {
        ErrorManager.getInstance().dispatch('Failed to upload profile picture. Please try again.');
      }
    });
  }
}
